...
`plugin/desk-panel/` 是一个小型 **dsh 客户端插件**：借 dsh 官方插件机制（`settings.section` + `sidebar.footer.action` 槽位 + `dsh plugin`）在 dsh「设置」中加入一组团队页——**「工作台用量」**（我的用量）、**「任务板」**（团队待办）、**「知识库」**（共享知识检索）、**「公司盘」**（共享文件：浏览/下载/上传）、**「成员管理」**（成员与模型通道，管理员）、**「通知」**（通知桥 + 定时提醒，管理员）、**「运维」**（服务/探活/备份/磁盘，管理员），并在**侧栏底部挂「📢 公告」**（团队公告 + 意见反馈，带未读徽标）——**不改 dsh 源码、零构建、零 npm 依赖**。按实例挂载：
